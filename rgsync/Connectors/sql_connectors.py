...
class MySqlConnection(BaseSqlConnection):
    ...
    def _getConnectionStr(self):
        return f"mysql+pymysql://{self.user}:{self.passwd}@{self.db}"
...
class PostgresConnection(BaseSqlConnection):
    ...
    def _getConnectionStr(self):
        return f"postgresql://{self.user}:{self.passwd}@{self.db}"
...
class SQLiteConnection(BaseSqlConnection):
    ...
    def _getConnectionStr(self):
        return f"sqlite:////{self.filePath}?check_same_thread=False"
...
class OracleSqlConnection(BaseSqlConnection):
    ...
    def _getConnectionStr(self):
        return f"oracle://{self.user}:{self.passwd}@{self.db}"
...
class MsSqlConnection(BaseSqlConnection):
    ...
    def _getConnectionStr(self):
        return "mssql+pyodbc://{user}:{password}@{server}:{port}/{db}?driver={driver}".format(
            user=self.user,
...
class SnowflakeSqlConnection(BaseSqlConnection):
    ...
    def _getConnectionStr(self):
        return f"snowflake://{self.user}:{self.passwd}@{self.account}/{self.db}"
...
class BaseSqlConnector:
    ...
    def WriteData(self, data):
        ...
        try:
            if not self.conn:
                ...
                if self.exactlyOnceTableName is not None:
                    ...
                    result = self.conn.execute(
                        self.sqlText(
                            f"select val from {self.exactlyOnceTableName} where id=:id"
                        ),
                        {"id": shardId},
                    )
                    res = result.first()
            ...
            msg = f'Failed connecting to SQL database, error="{str(e)}"'
            WriteBehindLog(msg)
        ...
        try:
            ...
            for d in data:
                ...
                self.shouldCompareId = False
                if (
                    op != OPERATION_UPDATE_REPLICATE
                ):  # we have only key name, it means that the key was deleted
                    if isAddBatch:
            ...
            if len(batch) > 0:
                ...
                if self.exactlyOnceTableName is not None:
                    self.conn.execute(
                        self.sqlText(self.exactlyOnceQuery),
                        {"id": shardId, "val": lastStreamId},
                    )
            trans.commit()
        except Exception as e:
            try:
...
class MySqlConnector(BaseSqlConnector):
    ...
    def PrepereQueries(self, mappings):
        def GetUpdateQuery(tableName, mappings, pk):
            ...
            values = [pk] + values
            values.sort()
            query = "%s (%s) VALUES (%s) ON DUPLICATE KEY UPDATE %s" % (
                query,
                ",".join(values),
                ",".join([":%s" % a for a in values]),
                ",".join(["%s=values(%s)" % (a, a) for a in values]),
            )

            return query

        self.addQuery = GetUpdateQuery(self.tableName, mappings, self.pk)
        self.delQuery = f"delete from {self.tableName} where {self.pk}=:{self.pk}"
        if self.exactlyOnceTableName is not None:
...
class PostgresConnector(BaseSqlConnector):
    ...
    def PrepereQueries(self, mappings):
        def GetUpdateQuery(tableName, mappings, pk):
            ...
            values = [
                f":{val}"
                for kk, val in ordered_mappings.items()
            ...
            update_stmts = [
                f"{v}=excluded.{v}"
                for v in ordered_mappings.values()
                if not v.startswith("_")
            ]
            query = f"""INSERT INTO {tableName} ({cols},{pk})
            VALUES ({value_stmt})
...
class SQLiteConnector(BaseSqlConnector):
    ...
    def PrepereQueries(self, mappings):
        def GetUpdateQuery(tableName, mappings, pk):
            query = f"INSERT OR REPLACE INTO {tableName}"
            values = [val for kk, val in mappings.items() if not kk.startswith("_")]
            values = [pk] + values
            values.sort()
            query = "%s(%s) values(%s)" % (
...
class OracleSqlConnector(BaseSqlConnector):
    ...
    def PrepereQueries(self, mappings):
        ...
        def GetUpdateQuery(table, pkey, values_with_pkey, values):
            merge_into = (
                "MERGE INTO %s d USING (SELECT 1 FROM DUAL) ON (d.%s = :%s)"
                % (table, pkey, pkey)
            )
            not_matched = "WHEN NOT MATCHED THEN INSERT (%s) VALUES (%s)" % (
                ",".join(values_with_pkey),
                ",".join([":%s" % a for a in values_with_pkey]),
            )
            matched = "WHEN MATCHED THEN UPDATE SET %s" % (
                ",".join(["%s=:%s" % (a, a) for a in values])
            )
            query = f"{merge_into} {not_matched} {matched}"
            return query

        self.addQuery = GetUpdateQuery(
            self.tableName, self.pk, values_with_pkey, values
        )
        self.delQuery = f"delete from {self.tableName} where {self.pk}=:{self.pk}"
        if self.exactlyOnceTableName is not None:
            self.exactlyOnceQuery = GetUpdateQuery(
                self.exactlyOnceTableName, "id", ["id", "val"], ["val"]
            )
...
class MsSqlConnector(BaseSqlConnector):
    ...
    def PrepereQueries(self, mappings):
        ...
        def GetUpdateQuery(tableName, mappings, pk):
            merge_into = (
                "MERGE %s AS Target USING (VALUES (:%s)) AS Source (key1) ON (Target.%s = Source.key1)"
                % (tableName, pk, pk)
            )
            not_matched = "WHEN NOT MATCHED BY TARGET THEN INSERT (%s) VALUES (%s)" % (
                ",".join(values_with_pkey),
                ",".join([":%s" % a for a in values_with_pkey]),
            )
            matched = "WHEN MATCHED THEN UPDATE SET %s" % (
                ",".join(["Target.%s=:%s" % (a, a) for a in values])
            )
            query = f"{merge_into} {not_matched} {matched};"
            return query
...
class SnowflakeSqlConnector(OracleSqlConnector):
    def __init__(self, connection, tableName, pk, exactlyOnceTableName=None):
        OracleSqlConnector.__init__(
            self, connection, tableName, pk, exactlyOnceTableName
        )


class DB2Connector(BaseSqlConnector):
    def __init__(self, connection, tableName, pk, exactlyOnceTableName=None):
        BaseSqlConnector.__init__(self, connection, tableName, pk, exactlyOnceTableName)
    ...
    def PrepereQueries(self, mappings):
        ...
        def GetUpdateQuery(table, pkey, values_with_pkey, values):
            merge_into = "MERGE INTO %s d USING (VALUES 1) ON (d.%s = :%s)" % (
                table,
                pkey,
                pkey,
            )

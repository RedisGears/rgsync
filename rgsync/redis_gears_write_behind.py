...
def ValidateHash(r):
    ...
    if value == None:
        ...
        if not (isinstance(r["value"], dict)):
            msg = 'Got a none hash value, key="%s" value="%s"' % (
                str(r["key"]),
                str(r["value"] if "value" in r.keys() else "None"),
            )
            WriteBehindLog(msg)
            raise Exception(msg)
    ...
    if operation not in OPERATIONS:
        msg = f'Got unknown operations "{operation}"'
        WriteBehindLog(msg)
        raise Exception(msg)
...
def UnregisterOldVersions(name, version):
    WriteBehindLog(f"Unregistering old versions of {name}")
    ...
    for registration in registrations:
        ...
        if "name" in desc.keys() and name in desc["name"]:
            ...
            if "version" not in desc.keys():
                ...
                WriteBehindLog(f"Unregistered {registrationDict['id']}")
                continue
            v = desc["version"]
...
def CreateAddToStreamFunction(self):
    def func(r):
            ...
            if op == OPERATION_UPDATE_REPLICATE:
                for kInHash, kInDB in self.mappings.items():
                    ...
                    if kInHash not in keys:
                        msg = "AddToStream: Could not find %s in hash %s" % (
                            kInHash,
                            r["key"],
                        )
                        WriteBehindLog(msg)
        ...
        execute(
            "xadd", self.GetStreamName(self.connector.TableName()), "*", *sum(data, [])
        )
            ...
            if operation == OPERATION_UPDATE_REPLICATE:
                for kInHash, kInDB in self.mappings.items():
                    if kInHash.startswith("_"):
                        continue
                    if kInHash not in keys:
                        msg = f"Could not find {kInHash} in hash {r['key']}"
                        WriteBehindLog(msg)
                        raise Exception(msg)
                    mappedValue[kInDB] = value[kInHash]
            func([{"value": mappedValue}])
        except Exception as e:
            WriteBehindLog(f"Failed writing data to the database, error='{str(e)}'")
...
class RGWriteBehind(RGWriteBase):
    def __init__(
        ...
        descJson = {
            "name": f"{name}.KeysReader",
            "version": version,
            "desc": f"add each changed key with prefix {keysPrefix}:* to Stream",
        }
        GB("KeysReader", desc=json.dumps(descJson)).map(transform).filter(
            ValidateHash
        ).filter(ShouldProcessHash).foreach(DeleteHashIfNeeded).foreach(
            CreateAddToStreamFunction(self)
        ).register(
            mode="sync",
            prefix=f"{keysPrefix}:*",
        ...
        descJson = {
            "name": f"{name}.StreamReader",
            "version": version,
            "desc": "read from stream and write to DB table %s"
            % self.connector.TableName(),
        }
        GB("StreamReader", desc=json.dumps(descJson)).aggregate(
            [], lambda a, r: a + [r], lambda a, r: a + r
        ).foreach(CreateWriteDataFunction(self.connector)).count().register(
            prefix=f"_{self.connector.TableName()}-stream-{UUID}-*",
            mode="async_local",
            batch=batch,
            duration=duration,
...
class RGWriteThrough(RGWriteBase):
    def __init__(self, GB, keysPrefix, mappings, connector, name, version=None):
        ...
        GB("KeysReader", desc=json.dumps(descJson)).map(PrepareRecord).filter(
            ValidateHash
        ).filter(WriteNoReplicate).filter(TryWriteToTarget(self)).foreach(
            UpdateHash
        ).register(
            mode="sync",
            prefix=f"{keysPrefix}*",
...
class RGJSONWriteBehind(RGWriteBase):
    ...
    def __init__(
        ...
        GB("KeysReader", desc=json.dumps(descJson)).filter(ValidateJSONHash).filter(
            ShouldProcessHash
        ).foreach(DeleteHashIfNeeded).foreach(CreateAddToStreamFunction(self)).register(
            mode="sync", prefix=f"{keysPrefix}:*", eventTypes=eventTypes
        )
        ...
        GB("StreamReader", desc=json.dumps(descJson)).aggregate(
            [], lambda a, r: a + [r], lambda a, r: a + r
        ).foreach(CreateWriteDataFunction(self.connector, dataKey)).count().register(
            prefix=f"_{self.connector.TableName()}-stream-{UUID}-*",
            mode="async_local",
            batch=batch,
            duration=duration,
...
class RGJSONWriteThrough(RGWriteBase):
    def __init__(
        self, GB, keysPrefix, connector, name, version=None, dataKey=GEARSDATAKEY
    ):
        mappings = {"sync_data": dataKey}
        RGWriteBase.__init__(self, mappings, connector, name, version)

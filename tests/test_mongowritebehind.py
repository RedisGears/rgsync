...
""" % (
    ...
    def _sampledata(self):
        d = {"some": "value", "and another": ["set", "of", "values"]}
        return d

    def _base_writebehind_validation(self):
        self.env.execute_command(
            "json.set", "person:1", ".", json.dumps(self._sampledata())
        )
    ...
    def testGiantWriteBehind(self):
        large_blob = {
            ...
            "phoneNumbers": [{"type": "home", "number": "1234567890"}],
...
class TestMongoWithConnectionString(TestMongoJSON):
    ...
    def setup_class(cls):
        ...
        con = f"mongodb://{dbuser}:{dbpasswd}@172.17.0.1:27017/"
        script = f"""
from rgsync import RGJSONWriteBehind, RGJSONWriteThrough
from rgsync.Connectors import MongoConnector, MongoConnection

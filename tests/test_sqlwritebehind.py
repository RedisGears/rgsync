...
"""
        ...
        e = create_engine(cls.connection(cls, **creds)).execution_options(
            autocommit=True
        )
    ...
    def testSimpleWriteBehind(self):
        ...
        self.env.execute_command(
            "hset", "person:1", "first_name", "foo", "last_name", "bar", "age", "22"
        )
    ...
    def testWriteBehindOperations(self):
        ...
        assert self.env.execute_command("hgetall", "person:1") == to_utf(
            {"first_name": "foo", "last_name": "bar", "age": "22"}
        )
    ...
    def testSimpleWriteThrough(self):
        ...
        assert self.env.execute_command("hgetall", "person:1") == to_utf(
            {"first_name": "foo", "last_name": "bar", "age": "20"}
        )
    ...
    def testSimpleWriteThroughPartialUpdate(self):
        ...
        assert self.env.execute_command("hgetall", "person:1") == to_utf(
            {"first_name": "foo1", "last_name": "bar", "age": "20"}
        )
    ...
    def testWriteTroughAckStream(self):
        ...
        self.env.execute_command(
            "hset __{person:1} first_name foo last_name bar age 20 # =1"
        )
        ...
        self.env.execute_command(
            "hset __{person:1} first_name foo last_name bar age 20 # ~2"
        )
...
class TestPostgresql(BaseSQLTest):
    def credentials(self):
        ...
        return {"dbuser": dbuser, "dbpasswd": dbpasswd, "db": db}
...
""" % (
        ...
        self.env.execute_command(
            "RG.PYEXECUTE", script, "REQUIREMENTS", pkg, "psycopg2-binary"
        )
...
""" % (
        ...
        self.env.execute_command(
            "RG.PYEXECUTE", script, "REQUIREMENTS", pkg, "pymysql[rsa]"
        )
...
""" % (
        ...
        self.env.execute_command(
            "RG.PYEXECUTE", script, "REQUIREMENTS", pkg, "ibm-db-sa"
        )
